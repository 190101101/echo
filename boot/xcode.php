...
function db()
{
    return library\triangle::getInstance();
}
...
/*user cookie*/
function cookie_has_user()
{
    return isset($_COOKIE['user']) ? TRUE : FALSE;
}

function cookie_user_token()
{
    return cookie_has_user() == TRUE ? json_decode($_COOKIE['user'])->user_token : FALSE;
}

/*user session*/
function notification()
{
    return isset($_SESSION['notice']) ? TRUE : FALSE;
}

function is_admin()
{
    return isset($_SESSION['user']) ? $_SESSION['user']['user_level'] >= 99 ? TRUE : FALSE : FALSE;
}
...
{
    return $_SESSION['user']['user_agreement'] ?? FALSE;
}
...
function user_token()
{
    return $_SESSION['user']['user_token'] ?? FALSE;
}

function user_created()
{
    return $_SESSION['user']['user_created'] ?? FALSE;
}

function user_coin()
{
    return $_SESSION['user']['user_coin'] ?? FALSE;
}

function textbelt_quota()
{
    return $_SESSION['user']['textbelt_quota'] ?? FALSE;
}

function user_referral()
{
    return $_SESSION['user']['user_referral'] ?? FALSE;
}

function user_status()
{
    return $_SESSION['user']['user_status'] ?? FALSE;
}

function user_quota()
{
    return $_SESSION['user']['user_quota'] ?? FALSE;
}
